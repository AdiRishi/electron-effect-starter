...
This decides between the two reference repos, which disagree: T3 Code colocates tests (`src/foo.test.ts`), while the Effect monorepo (vendored under `.repos/effect-smol`) keeps a separate per-package test directory. We take the Effect shape.
