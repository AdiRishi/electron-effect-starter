...
export const portFlag = Flag.integer("port").pipe(
  Flag.withDescription(
    "Port for the HTTP/WebSocket server (default 13773 or APP_SERVER_PORT).",
  ),
  Flag.optional,
);
...
export const devWebUrlFlag = Flag.string("dev-web-url").pipe(
  Flag.withDescription(
    "Dev web URL to redirect navigations to (equivalent to APP_DEV_WEB_URL).",
  ),
  Flag.optional,
);
export const bootstrapFdFlag = Flag.integer("bootstrap-fd").pipe(
  Flag.withDescription(
    "Read the one-time bootstrap envelope from the given file descriptor.",
  ),
  Flag.optional,
);
...
export const resolveServerConfig = Effect.fn("cli.resolveServerConfig")(
  function* (flags: CliServerFlags) {
